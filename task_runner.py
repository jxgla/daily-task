...
功能: 使用 YYDS Mail (215.im) 临时邮箱，并发账号，自动获取 OTP 验证码
"""
...
TOTAL_ACCOUNTS = 60
WORKER_COUNT = 3
YYDSMAIL_API_KEY = os.environ["YYDSMAIL_API_KEY"]
PROXY = ""
TOKEN_JSON_DIR = "codex"
...
DEFAULT_TOTAL_ACCOUNTS = TOTAL_ACCOUNTS
DEFAULT_WORKER_COUNT = WORKER_COUNT
DEFAULT_PROXY = PROXY.strip() or None

if not YYDSMAIL_API_KEY:
    print("⚠️ 警告: 未设置 YYDSMAIL_API_KEY，请先在脚本开头填写")

# 全局线程锁
_print_lock = threading.Lock()
_file_lock = threading.Lock()

_IMPORTANT_LOG_MARKERS = (
    "创建临时邮箱",
    "开始获取 Codex Token",
    "验证码:",
    "Token 已保存",
...
# 域名缓存，避免每次创建邮箱都请求
_yydsmail_domains_cache = []
_yydsmail_domains_lock = threading.Lock()


def _fetch_yydsmail_domains():
    """获取 YYDS Mail 可用域名列表（带缓存）"""
    global _yydsmail_domains_cache
    with _yydsmail_domains_lock:
        if _yydsmail_domains_cache:
            return _yydsmail_domains_cache
    try:
        headers = {}
        if YYDSMAIL_API_KEY:
            headers["X-API-Key"] = YYDSMAIL_API_KEY
        session = curl_requests.Session()
        res = session.get(f"{YYDSMAIL_API_BASE}/domains", headers=headers, timeout=15)
        if res.status_code == 200:
            ...
            if domain_names:
                with _yydsmail_domains_lock:
                    _yydsmail_domains_cache = domain_names
                return domain_names
    except Exception as e:
        print(f"⚠️ 获取 YYDS Mail 域名列表失败: {e}")
    return []
...
def create_temp_email():
    ...
    try:
        payload = {"address": prefix, "domain": domain}
        res = session.post(
            f"{YYDSMAIL_API_BASE}/accounts",
            json=payload,
            headers=headers,
            timeout=15,
        ...
        if not mail_token:
            raise Exception(f"创建邮箱成功但未返回 token, 响应: {resp}")
...
def _fetch_emails_yydsmail(mail_token: str):
    """从 YYDS Mail 获取邮件列表"""
    try:
...
def _fetch_email_detail_yydsmail(mail_token: str, msg_id):
    """获取 YYDS Mail 单封邮件详情"""
    try:
        headers = {"Authorization": f"Bearer {mail_token}"}
        session = _create_yydsmail_session()
...
def wait_for_verification_email(mail_token: str, timeout: int = 120):
    ...
    while time.time() - start_time < timeout:
        messages = _fetch_emails_yydsmail(mail_token)
        if messages and len(messages) > 0:
            first_msg = messages[0]
            msg_id = first_msg.get("id")

            if msg_id:
                detail = _fetch_email_detail_yydsmail(mail_token, msg_id)
                if detail:
...
class ChatGPTRegister:
    ...
    def _print(self, msg):
        if not VERBOSE_LOG and not any(marker in msg for marker in _IMPORTANT_LOG_MARKERS):
            return
        prefix = f"[{self.tag}] " if self.tag else ""
        with _print_lock:
            print(f"{prefix}{msg}")
    ...
    def _fetch_email_detail_yydsmail(self, mail_token: str, msg_id):
        """获取 YYDS Mail 单封邮件详情"""
        try:
            headers = {"Authorization": f"Bearer {mail_token}"}
            session = self._create_yydsmail_session()
...
def run_batch(total_accounts: int = 3, max_workers=3, proxy=None):
    """并发批量注册 - YYDS Mail 临时邮箱版"""

    if not YYDSMAIL_API_KEY:
        print("❌ 错误: 未设置 YYDSMAIL_API_KEY")
        print("   请先在脚本开头填写 YYDSMAIL_API_KEY")
        return
    ...
    if ENABLE_OAUTH:
        ...
        print(f"  Token输出目录: {TOKEN_JSON_DIR}/")
    print(f"{'#'*60}\n")
    ...
    if success_count > 0:
        print(f"  JSON输出目录: {TOKEN_JSON_DIR}/")
    print(f"{'#'*60}")


def main():
    print("=" * 60)
    print("  Chat工具 (YYDS Mail 临时邮箱版)")
    print("=" * 60)

    if not YYDSMAIL_API_KEY:
        print("\n❌ 错误: 未设置 YYDSMAIL_API_KEY")
        print("   请先在脚本开头填写 YYDSMAIL_API_KEY")
        return
